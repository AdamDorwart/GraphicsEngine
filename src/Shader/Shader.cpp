...
char* Shader::readFile(const char *filename)
{
	...
	if(!fp) {
		LogError("File doesn't exist [%s]", (std::string(SHADER_PATH) + filename).c_str());
        exit(EXIT_FAILURE);
    }
...
bool Shader::addShader(GLenum ShaderType, const char* pFilename)
{
	...
	if (ShaderObj == 0) {
		LogError("Error creating shader type %d\n", ShaderType);
		return false;
	}
...
bool Shader::finalize()
{
	...
	if (Success == 0) {
		...
		LogError("Error linking shader program: '%s'\n", ErrorLog);
		return false;
	}
...
GLint Shader::getUniformLocation(const char* pUniformName)
{
	...
	if (Location == -1) {
		LogError("Warning! Unable to get the location of uniform '%s'\n", pUniformName);
	}
